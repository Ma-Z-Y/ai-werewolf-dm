...
本文件是 S3 的前端产品宪法，状态为 `frozen`。用户已于 2026-09-25 接受
五项决策及其精确边界。后续 S3 设计、实施计划和代码必须从本文件出发。
...
4. 在活动投票投影中加入安全的提交进度。
5. 填充已存在但此前未写入的 `public_timeline`，使公共阶段、死亡、投票和
   终局结果有确定性公开文案。
...
11. `public_timeline` 只包含公开事件，平安夜、平票、超时、暂停、恢复和
    终局均有确定性公开文案，且不改变 `state_hash()`。
...
- 在用户授权整体复核通过的目标下，将 S3-P0-05 纳入既有的
  `public_timeline` 契约补全。
- 不新增公开字段，不改变领域状态 hash；只让已有公开字段实际产生确定性、
  已脱敏的公开文本。
...
- 用户接受后由 `v0.1.0 proposed` 升级为 `v1.0.0 frozen`。
